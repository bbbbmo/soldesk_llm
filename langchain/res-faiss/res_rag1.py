...
# 현재 파이썬 스크립트 실행 같은 위치에 있는 "faiss-cpu" 폴더 경로
r_faiss = os.path.join(current_dir, "faiss-cpu")

# TextLoader 클래스를 사용하여 "restaurant.txt"라는 파일에서 텍스트를 로드합니다.
loader = TextLoader(os.path.join(current_dir, "restaurants.txt"),encoding='utf-8' )
...
# 생성된 FAISS 인덱스를 나중에 사용할 수 있도록 "faiss-cpu"라는 로컬 디렉토리에 저장합니다.
db.save_local(r_faiss)
print("레스토랑 임베딩 저장 완료", r_faiss)
