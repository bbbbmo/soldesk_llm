...
async def main():
    ...
    # allow_dangerous_deserialization=True 옵션은 역직렬화를 허용합니다. -> 파일을 메모리로 올림
    load_db = FAISS.load_local(f'{current_dir}/faiss-cpu', embeddings, allow_dangerous_deserialization=True)

    # 검색할 쿼리 문자열을 정의합니다.
    query = "음직점의 공간은 예약 없이도 이용 가능한가요?"
