...
prompt3 = PromptTemplate.from_template(
    "다음 숙박 시설 리뷰를 읽고 0~10까지 부정/긍정 점수 매기세요. 숫자로.\n\n{translation}"
)

# 프롬프트 4: 원본 리뷰 언어 식별
prompt4 = PromptTemplate.from_template(
    "다음 숙박 시설 리뷰에 사용된 언어가 무엇인가요? 언어 이름만 알려주세요.\n\n{review}"
)
...
# 단계 1: 리뷰 번역 체인
translate_chain = prompt1 | openai_llm | StrOutputParser()

# 단계 2: 번역된 리뷰 요약 체인
summarize_chain = prompt2 | openai_llm | StrOutputParser()

# 단계 3: 감성 점수 평가 체인
score_chain = prompt3 | openai_llm | StrOutputParser()

# 단계 4: 언어 식별 체인
language_chain = prompt4 | openai_llm | StrOutputParser()

# 단계 5: 첫 번째 답변 생성 체인
reply1_chain = prompt5 | openai_llm | StrOutputParser()

# 단계 6: 두 번째 답변 (한국어 번역) 생성 체인
reply2_chain = prompt6 | openai_llm | StrOutputParser()
...
# 체인 실행 및 결과 출력
try:
   result = combined_lcel.invoke(input={"review":review_text})
   # 결과 딕셔너리에서 각 키를 사용하여 값을 출력합니다.N/A(Not Available OR Not Applicable)
   print(f"리뷰번역:{result.get('translation','N/A')}\n")
   print(f"리뷰요약:{result.get('summary','N/A')}\n")
   print(f"평가점수:{result.get('score','N/A')}\n")
   print(f"리뷰언어:{result.get('language','N/A')}\n")
   print(f"영문답변:{result.get('reply1','N/A')}\n")
   print(f"한글답변:{result.get('reply2','N/A')}\n")
